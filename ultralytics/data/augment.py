...
class RandomTargetMasking:
    ...
    def __call__(self, labels):
        ...
        for i in range(len(cls)):
            if np.random.rand() < self.p and cls[i] in [0, 1, 3, 5, 7]:
        ...
        for i in range(len(selected_bboxes)):
            ...
            center_x = (x1 + x2) // 2
            center_y = (y1 + y2) // 2
            # TODO how to select the size of the mask?
            # There are two ways:
            # 1. select a half of the bbox area randomly (this is the current implementation)
            # 2. select 1/4 of the bbox area randomly
...
class MixUpPlus(BaseMixTransform):
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0, class_probs=None, img_labels=None) -> None:
        super().__init__(dataset=dataset, pre_transform=pre_transform, p=p)
    ...
    def get_indexes(self):
        ...
        indices = [i for i, img_labels_i in enumerate(self.img_labels) if chosen_cls in img_labels_i]
...
def v8_transforms_plus(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose([
        pre_transform,
        MixUpPlus(dataset, pre_transform=pre_transform, p=hyp.mixup, class_probs=class_probs, img_labels=img_labels),
        Albumentations(p=1.0),
        RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
        RandomFlip(direction='vertical', p=hyp.flipud),
        RandomFlip(direction='horizontal', p=hyp.fliplr, flip_idx=flip_idx)])  # transforms
